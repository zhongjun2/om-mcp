...
**时间戳计算规则（重要，防止年份错误）：**
- **禁止心算时间戳**，必须用以下命令验证后再使用：
  ```bash
  python3 -c "from datetime import datetime, timezone; dt=datetime(YYYY,MM,DD,tzinfo=timezone.utc); print(int(dt.timestamp()*1000))"
  ```
- 2026 年各月边界参考（UTC 毫秒时间戳）：

  | 月份 | 月初（00:00:00） | 月末（23:59:59） |
  |------|----------------|----------------|
  | 1月  | `1767225600000` | `1769903999000` |
  | 2月  | `1769904000000` | `1772323199000` |
  | 3月  | `1772323200000` | `1775001599000` |
  | 4月  | `1775001600000` | `1777593599000` |
...
- **报告完整性检查（保存前必须逐项核对）**：报告必须包含以下全部 13 项，数据不可用时写"暂无数据"或"API错误"，不得整节省略：
  ...
  | 4 | 领域主流项目适配、集成、引用度 | ✅ |
  | 5 | TOP开发者留存率 | ✅ |
  | 6 | YTD社区下载量（当月+YTD+环比） | ✅ |
  | 7 | Issue平均/中位首次响应时长（天） | ✅ |
  | 8 | Issue平均/中位关闭时长（天） | ✅ |
  | 9 | 论坛平均/中位首次响应时长（天） | ✅ |
  | 10 | 论坛平均/中位关闭时长（天） | ✅ |
  | 11 | 版本稳定发布偏差 | ✅ |
